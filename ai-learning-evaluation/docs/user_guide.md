...
Run `./run_app.ps1` on Windows or follow the README manual commands. The app opens with a client export from `data/client/` if present, otherwise the included synthetic file; no upload or credentials are required.

## Workflow

1. Import a Qualtrics-compatible CSV or retain the auto-loaded dataset.
...
9. Enter the reviewer name, verify the evidence/privacy/wording checklist, and explicitly confirm the review. The Evidence check line must show no unsupported claims — Approve & export is refused otherwise.
