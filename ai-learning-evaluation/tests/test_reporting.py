...
def test_report_contains_required_sections_and_draft_status(golden_df):
    report = generate_report(context(golden_df), "facilitator", LocalDemoProvider())
...
def test_audience_reports_differ(golden_df):
    ...
    facilitator = generate_report(ctx, "facilitator", LocalDemoProvider()).content
    client = generate_report(ctx, "client", LocalDemoProvider()).content
...
def test_approval_changes_status(golden_df):
    draft = generate_report(context(golden_df), "client", LocalDemoProvider())
...
def test_exports_are_nonempty_and_docx_is_valid_zip(golden_df):
    report = generate_report(context(golden_df), "client", LocalDemoProvider()).content
